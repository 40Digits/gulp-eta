...
module.exports = function(base, options) {
  ...
  // Srcs for gulp.src
  srcs.images     = p.join(source.images, config.globs.images);
  srcs.scripts    = p.join(source.scripts, config.globs.scripts);
  srcs.sprites    = p.join(source.sprites, config.globs.sprites);
  srcs.styles     = p.join(source.styles, config.globs.styles);
  srcs.symbols    = p.join(source.symbols, config.globs.symbols);
  srcs.static     = p.join(source.static, '*.tpl.html');
	...
	// Sprites
	config.sprites = {
		destSass: p.join(source.styles, 'utils'),
		settings: {
			src: srcs.sprites,
			retina: true,
			dimension: [
			...
			style: '_sprites.scss',
			cssPath: p.join(config.scaffold.assets.root, config.scaffold.assets.sprites),
	...
	// Symbols
	config.symbols = {
		tplCss: p.join(source.symTpl, 'symbols.tpl.css'),
		tplSass: p.join(source.symTpl, 'symbols.tpl.scss'),
		tplHtml: p.join(source.symTpl, 'symbols.tpl.html'),
		fontPath: p.join(config.scaffold.assets.root, config.scaffold.assets.symbols, '/'),
		destSass: p.join(source.styles, 'utils'),
		settings: {
			fontName: 'symbols',
